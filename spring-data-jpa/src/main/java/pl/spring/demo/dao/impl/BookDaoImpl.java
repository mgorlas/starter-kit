...
public class BookDaoImpl implements BookDao {
    ...
    public List<BookEntity> findBookByTitle(String title) {
    	...
    	for (BookEntity bookEntity : ALL_BOOKS) {
			if(title.equalsIgnoreCase(bookEntity.getTitle())){
    ...
    @Override
    public List<BookEntity> findBooksByAuthor(AuthorTo author) {
    	List<BookEntity> foundBooks = new ArrayList<>();
    	for (BookEntity bookEntity : ALL_BOOKS) {
    		for (AuthorTo authorTo : bookEntity.getAuthors()) {
    			if((authorTo.getFirstName()).equalsIgnoreCase(author.getFirstName()) && (authorTo.getLastName()).equalsIgnoreCase(author.getLastName())){
    				foundBooks.add(bookEntity);
    			}
			}
		}
